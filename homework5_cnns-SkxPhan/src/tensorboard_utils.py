...
class ImageLabelingLogger(tf.keras.callbacks.Callback):
    ...
    def log_image_labels(self, epoch_num, logs):
        ...
        misclassified_path = (
            "misclassified" + self.logs_path[self.logs_path.index(os.sep) :]
        )
        if not os.path.exists(misclassified_path):
            os.makedirs(misclassified_path)
        for correct, wrong, img in zip(
            correct_labels, wrong_labels, misclassified
        ):
            ...
            plt.imsave(
                misclassified_path + os.sep + correct + os.sep + image_name, img
            )

        with file_writer_il.as_default():
            tf.summary.image(
                "0 Example Set of Image Label Predictions (blue is correct; red is incorrect)",
                figure_img,
                step=epoch_num,
            )
            for label, wrong, img in zip(
                correct_labels, wrong_labels, misclassified
            ):
                ...
                tf.summary.image(
                    ...
                    + str(epoch_num)
                    + ": "
                    + self.datasets.idx_to_class[label]
                    + " misclassified as "
                    + self.datasets.idx_to_class[wrong],
                    img,
                    step=epoch_num,
                )
    ...
    def plot_confusion_matrix(self, cm, class_names):
        ...
        for i in range(cm.shape[0]):
            for j in range(cm.shape[1]):
                ...
                plt.text(
                    j, i, cm[i, j], horizontalalignment="center", color=color
                )
...
class CustomModelSaver(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if cur_acc > max_acc:
            ...
            # minimum weight
            if (
                self.max_num_weights > 0
                and num_weights + 1 > self.max_num_weights
            ):
    ...
    def scan_weight_files(self):
        ...
        for weight_file in files:
            if weight_file.endswith(".h5"):
                num_weights += 1
                file_acc = float(
                    re.findall(r"[+-]?\d+\.\d+", weight_file.split("acc")[-1])[
                        0
                    ]
                )
                if file_acc > max_acc:
