...
parser.add_argument(
    "--data", default="../../data", help="Dataset relative filepath"
)
...
def main():
    ...
    if ARGS.dataset == "scenerec":
        train_images, train_labels, test_images, test_labels = (
            format_data_scene_rec(ARGS.data, hp)
        )
        num_classes = hp.scene_class_count
    else:
        train_images, train_labels, test_images, test_labels = (
            format_data_mnist(ARGS.data)
        )
        num_classes = hp.mnist_class_count

    model = Model(
        train_images, train_labels, num_classes, hp, test_images, test_labels
    )
