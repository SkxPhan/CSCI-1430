...
image = transform.rescale(
    color.rgb2gray(io.imread("questions/images/LaddObservatory2.jpg")), 0.25
)

harris_response = corner_harris(image)
# Note: Feel free to play with these parameters to investigate their effects
# coords = peak_local_max(harris_response, min_distance=10, threshold_rel=0.05)
coords = corner_peaks(harris_response, min_distance=10, threshold_rel=0.05)
