...
def create_results_webpage(
    ...
    with open("results_webpage/index.html", "w+") as f:
        ...
        f.write(
            "<th colspan=%d>False positives with true label</th>\n"
            % num_samples
        )
        f.write(
        ...
        for i, cat in enumerate(categories):
            ...
            true_positives = np.take(
                ...
                np.argwhere(
                    np.logical_and(
                        test_labels == cat, predicted_categories == cat
                    )
                ),
            )
            ...
            false_negative_labels = np.take(
                predicted_categories, false_negative_inds
            )
            ...
            train_examples = train_examples[
                0 : min(len(train_examples), num_samples)
            ]
            true_positives = true_positives[
                0 : min(len(true_positives), num_samples)
            ]
            ...
            for j in range(num_samples):
                if j + 1 <= len(false_positives):
                    ...
                    f.write(
                        "<br><small>%s</small>" % false_positive_labels[j][0]
                    )
                    f.write("</td>\n")
                else:
                    f.write("<td bgcolor=LightCoral>")
            ...
            for j in range(num_samples):
                if j + 1 <= len(false_negatives):
                    ...
                    f.write(
                        "<br><small>%s</small>" % false_negative_labels[j][0]
                    )
                    f.write("</td>\n")
                else:
                    f.write("<td bgcolor=#FFBB55>")
