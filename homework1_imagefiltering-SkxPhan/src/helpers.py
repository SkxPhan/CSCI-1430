    ...
    for scale in range(2, scales + 1):
        ...
        output = np.hstack(
            (
                output,
                np.ones(
                    (original_height, padding, num_colors), dtype=np.float32
                ),
            )
        )
        # downsample image
        cur_image = rescale(
            cur_image, scale_factor, mode="reflect", multichannel=True
        )
...
def equalize_image_sizes(im_one, im_two):
    assert (
        im_one.shape[2] == im_two.shape[2]
    ), "the third dimension of these images do not match"
    ...
    im_one = resize(
        im_one,
        (
            int(im_one.shape[0] - x_resize),
            int(im_one.shape[1] - y_resize),
            im_one.shape[2],
        ),
    )
    im_two = resize(
        im_two,
        (
            int(im_two.shape[0] + x_resize),
            int(im_two.shape[1] + y_resize),
            im_two.shape[2],
        ),
    )
