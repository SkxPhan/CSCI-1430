...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Homework 3 Camera Geometry")
    parser.add_argument(
        "--sequence",
        required=False,
        default="cards",
    ...
    parser.add_argument(
        "--visualize-ransac", action="store_true", help="Visualizes Ransac"
    )
...
def main():
    ...
    for i in range(len(images) - 1):
        ...
        print(
            f"Getting matches for images {i + 1} and {i + 2} of {len(images)}..."
        )
        ...
        points3d_found, inliers1_from3d, inliers1_from3d = (
            student.matches_to_3d(inliers1, inliers2, M1, M2)
        )
        points3d += points3d_found.tolist()
        points3d_color += [
            tuple(image1[int(point[1]), int(point[0]), :] / 255.0)
