...
def compute_extrinsic_matrix(tx, ty, tz, alpha, beta, gamma):
    rotation_matrix = compute_rotation_matrix(alpha, beta, gamma)
    return np.column_stack((rotation_matrix, [tx, ty, tz]))
...
def calculate_camera_matrix(tx, ty, tz, alpha, beta, gamma, fx, fy, skew, u, v):
    ...
    initial_intrinsic_matrix_to_replace = np.array(
        [
            [fx, skew, u],
            [0, fy, v],
            [0, 0, 1],
    ...
    initial_extrinsic_matrix_to_replace = compute_extrinsic_matrix(
        tx, ty, tz, alpha, beta, gamma
    )

    initial_camera_matrix_to_replace = (
...
axrotZ = plt.axes(
    [slider_offset_X, spacing + slider_offset_Y, slider_width, slider_height]
)
rotZ_slider = Slider(
    ax=axrotZ,
    label="z-rotation",
...
for row in range(0, 3):
    for col in range(0, 4):
        rtText[row][col] = plt.text(
            rtLabelX + 0.4 + col * 0.5,
            rtLabelY - row * 0.10,
            "{:.2f}".format(0),
            size="12",
        )
...
camera_matrix, intrinsic_matrix, extrinsic_matrix = calculate_camera_matrix(
    transX_slider.val,
    transY_slider.val,
    transZ_slider.val,
    rotX_slider.val,
...
def update(val):
    ...
    for row in range(0, 3):
        for col in range(0, 3):
            kText[row][col].set_text(
                "{:.2f}".format(intrinsic_matrix[row][col])
            )

    # Update text labels for extrinsic matrix
    for row in range(0, 3):
        for col in range(0, 4):
            rtText[row][col].set_text(
                "{:.2f}".format(extrinsic_matrix[row][col])
            )
